...
def test_exif_crud(client_and_user_1, cleaner):
    ...
    with client as client:
        ...
        # 3. Проверка того, что у неё нет EXIF
        ...
        exif_response_2 = api_create_exif_v1_exif_item_uuid_post.sync(item_uuid,
                                                                      client=client, body=body_1)
        assert exif_response_2['result'] == 'created exif'
        assert exif_response_2['item_uuid'] == str(item_uuid)

        # 5. Проверка того, что у неё есть EXIF
        ...
        exif_response_4 = api_update_exif_v1_exif_item_uuid_put.sync(item_uuid,
                                                                     client=client, body=body_2)
        ...
        # 7. Проверка того, что EXIF сохранился
        ...
        # 9. Проверка того, что у неё нет EXIF
